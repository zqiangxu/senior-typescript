...
type BestGetMiddleElement<T extends readonly unknown[]> = T['length'] extends 0 | 1 | 2
  ...
  ? BestGetMiddleElement<Middle>
  : never;

type bSimple1 = BestGetMiddleElement<[1, 2, 3, 4, 5]>; // expected to be [3]
type bSimple2 = BestGetMiddleElement<[1, 2, 3, 4, 5, 6]>; // expected to be [3, 4]
type bSimple3 = BestGetMiddleElement<[1, 2, 3, 4]>; // expected to be [2, 3]
type bSimple4 = BestGetMiddleElement<[1, 2, 3]>; // expected to be [2]
type bSimple5 = BestGetMiddleElement<[1, 2]>; // expected to be [1, 2]
type bSimple6 = BestGetMiddleElement<[1]>; // expected to be [1]
type bSimple7 = BestGetMiddleElement<[never]>;
type bSimple8 = BestGetMiddleElement<[() => string, () => number]>;
